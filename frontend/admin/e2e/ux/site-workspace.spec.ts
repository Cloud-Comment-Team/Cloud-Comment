...
test('мастер хранит черновик, проверяет шаги и использует реальный preview', async ({ page }) => {
  ...
  await page.getByRole('spinbutton', { name: 'Лимит ссылок' }).focus()
  await expect.poll(() => page.evaluate((draftKey) => {
    const draft = JSON.parse(localStorage.getItem(draftKey) ?? '{}') as { currentStep?: number; focusTarget?: string }
    return `${draft.currentStep}:${draft.focusTarget}`
  }, OWNER_DRAFT_KEY)).toBe('1:automodMaxLinks')
  await page.reload()
  await expect(page.getByRole('textbox', { name: /Разрешённые origins/ })).toHaveValue('https://draft.example.com')
...
test('рабочее пространство разделяет установку, настройки и опасную зону', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Краткая сводка сайта' })).toHaveCount(0)
  ...
  await page.getByRole('button', { name: 'Установка', exact: true }).click()
  await expect(page).toHaveURL(new RegExp(`/sites/${SITE_ID}\\?section=installation$`))
  ...
  await expect(page).toHaveURL(new RegExp(`/sites/${SITE_ID}\\?section=appearance$`))
  ...
  await page.getByRole('combobox', { name: 'Состояние' }).selectOption('empty')
  await expect(page.frameLocator('iframe[title="Предварительный просмотр виджета"]').getByText('Пока нет комментариев.')).toBeVisible()
  await expectNoSeriousAccessibilityViolations(page)

  await page.getByRole('button', { name: 'Модерация', exact: true }).click()
  await expect(page).toHaveURL(new RegExp(`/sites/${SITE_ID}\\?section=moderation$`))
  ...
  await expect(page).toHaveURL(new RegExp(`/sites/${SITE_ID}\\?section=origins$`))
  ...
  await expect(page).toHaveURL(new RegExp(`/sites/${SITE_ID}\\?section=danger$`))

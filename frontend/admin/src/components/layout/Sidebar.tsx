...
const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 transform border-r transition-transform duration-200 ease-in-out lg:translate-x-0 ${
      ...
      >
        <div className="flex h-full flex-col">
          <div className="flex h-14 items-center gap-3 border-b px-4" style={{ borderColor: 'var(--border)' }}>
            <div
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
            {navigation.map((item) => (
              <NavLink
                ...
                onClick={onClose}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition hover:opacity-85"
                style={({ isActive }) => ({

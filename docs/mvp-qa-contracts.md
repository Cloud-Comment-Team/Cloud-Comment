...
17. Moderation endpoints используют `/api/moderation/**`.
18. Public widget endpoints используют `/api/public/**`.
19. Response shapes comment / pagination / moderation action совпадают с разделом выше.

### Docs / architecture alignment

20. Route names в этом документе, C4 dynamic views и реализованных controllers совпадают по префиксу `/api/`.
21. C4 container names (`adminWebApp`, `widgetRuntime`, `backendApi`, `reverseProxy`) не расходятся с README и deployment diagram.

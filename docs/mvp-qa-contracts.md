...
8. `/api/public/sites/**` не использует admin cookie. Новый iframe передаёт page-bound widget context во всех site-запросах, а действия пользователя дополнительно требуют bearer-сессию доменной audience `WIDGET` (в базе `WIDGET_FRAME`), ограниченную site+origin. Bearer можно использовать с новым page context только в том же site+origin; плохой bearer не понижает запрос до анонимного.
...
| --- | --- | --- | --- |
| `/api/privacy/consent-requirements` | `GET` | legacy cross-origin без credentials | Версии и URL документов для регистрации в админке; CORS разрешён только для этого совместимого GET-контракта |
| `/api/public/sites/{siteId}/privacy/consent-requirements` | `GET` | widget context | Те же документы для регистрации внутри iframe |

Widget register uses the same request body as admin register (see below).

#### Получение требований согласия

Глобальный `GET /api/privacy/consent-requirements` сохраняет ограниченную legacy-совместимость: публичный cross-origin `GET` без credentials, с разрешённым клиентским заголовком только `Accept`, без изменяющих методов. Новый виджет читает только site-scoped alias с действующим `X-CloudComment-Widget-Context`; dedicated frame не обращается к глобальному privacy API.

Относительные URL юридических документов разрешаются внутри iframe относительно его физического CloudComment origin, никогда относительно embedding page. Виджет открывает в новой вкладке только HTTP(S) пути `/legal/*` того же frame origin с `noopener noreferrer`; host-страница не участвует в переходе и не получает URL с секретом.
...
| Маршрут | Метод | Доступ | Назначение |
| --- | --- | --- | --- |
| `/api/public/sites/{siteId}/widget-frame` | `GET` | активный сайт | Динамический no-store shell iframe с индивидуальным CSP `frame-ancestors` |
| `/api/public/sites/{siteId}/widget-context/bootstrap` | `POST` | разрешённый parent origin, наблюдаемый браузером | Связать публичный P-256 ключ и каноническую страницу с двухминутным ticket |
| `/api/public/sites/{siteId}/widget-context/exchange` | `POST` | точный выделенный frame origin | Атомарно обменять P1363 proof на двухчасовой context |
| `/api/public/sites/{siteId}/config` | `GET` | widget context; для старого анонимного чтения сохранена проверка parent origin | Вернуть публичную конфигурацию виджета |
| `/api/public/sites/{siteId}/privacy/consent-requirements` | `GET` | widget context | Вернуть версии и URL юридических документов CloudComment |
| `/api/public/sites/{siteId}/auth/register` | `POST` | widget context | Создать аккаунт посетителя из изолированного iframe |
| `/api/public/sites/{siteId}/auth/login` | `POST` | widget context | Создать scoped bearer виджета |
| `/api/public/sites/{siteId}/auth/me` | `GET` | widget context + scoped bearer | Вернуть профиль текущего посетителя |
| `/api/public/sites/{siteId}/auth/logout` | `POST` | widget context + scoped bearer | Отозвать текущую сессию посетителя |
| `/api/public/sites/{siteId}/pages/comments` | `GET` | widget context; для старого анонимного чтения сохранена проверка parent origin | Вернуть комментарии связанной страницы |
| `/api/public/sites/{siteId}/pages/comments` | `POST` | widget context + scoped bearer | Создать комментарий авторизованного посетителя |
| `/api/public/sites/{siteId}/comments/{commentId}` | `PATCH` | widget context + scoped bearer | Изменить свой комментарий на связанной странице |
| `/api/public/sites/{siteId}/comments/{commentId}` | `DELETE` | widget context + scoped bearer | Мягко удалить свой комментарий на связанной странице |
| `/api/public/sites/{siteId}/comments/{commentId}/reaction` | `PUT` | widget context + scoped bearer | Установить или снять свою реакцию на связанной странице |

Контракт изоляции:

1. Iframe создаёт неизвлекаемую пару ECDSA P-256 и передаёт host-loader только публичный SPKI в Base64 URL без padding. Host вызывает bootstrap с `{ publicKey, pageUrl }`; backend берёт parent origin только из браузерного `Origin`, проверяет exact allowed-origin и возвращает `{ ticket, expiresAt, canonicalPageUrl, publicKeyFingerprint }`.
2. Loader один раз передаёт ответ внутрь iframe через выделенный `MessageChannel`. Frame подписывает P1363 raw-64 подписью UTF-8 payload с точными переводами строк:
...
3. Exchange принимает `{ ticket, proof }` только с точного выделенного `widget`-origin. `Origin: null` и same-origin frame отклоняются. Ticket живёт две минуты, поглощается атомарно и не может быть использован повторно.
4. Ответ exchange `{ contextToken, expiresAt }` живёт два часа. Каждый frame-запрос к site API передаёт `X-CloudComment-Widget-Context`; page-scoped маршруты также передают `X-CloudComment-Page-Url`. Авторизованные запросы добавляют `Authorization: Bearer <widget-token>`; сессия живёт не более 60 минут, ограничена site и parent origin и может использоваться с новым page context только в том же scope.
5. Bootstrap, exchange, context-protected ответы и ошибки имеют `Cache-Control: no-store`. Context и bearer не передаются host-странице, не попадают в URL, DOM, события общего `window` или `postMessage` без закрытого порта.
6. Dedicated frame хранит context и bearer только в собственном origin-scoped `sessionStorage`: context-ключ ограничен site, parent origin и страницей, bearer-ключ — site и parent origin. Reload валидирует context перед повторным использованием. `destroy()` очищает page context, но сохраняет bearer для SPA-навигации того же scope; logout и подтверждённый `401` очищают оба значения. За 30 секунд до серверного срока frame инициирует rebootstrap с учётом заголовка `Date`.
...
- Parent `Origin` обязателен на bootstrap и должен exact-match одну из записей `site_allowed_origins` активного сайта. Последующие frame-запросы имеют transport origin выделенного iframe либо `null`; исходный parent origin берётся только из серверного context и повторно сверяется с актуальной политикой сайта.
- `pageUrl` должен быть absolute `http/https` URL того же origin, что и parent origin в context. До bootstrap, поиска или создания страницы backend удаляет fragment и приватные query-параметры по таблице ниже. `X-CloudComment-Page-Url` обязан совпадать с page hash контекста; reply, reaction, edit и delete дополнительно проверяют принадлежность `commentId` этой же странице.
...
- Embedded widget auth использует только context-protected `/api/public/sites/{siteId}/auth/*`, а не `/api/auth/*`; admin cookie в iframe не читается и не отправляется.
- Юридический контракт виджет получает только через context-protected `/api/public/sites/{siteId}/privacy/consent-requirements`. Экспорт и удаление аккаунта доступны только в админке через глобальные `/api/account/*`; WIDGET bearer эти маршруты не открывает.
...
| Surface | Contract |
| --- | --- |
| `/widget/cloud-comment-widget.js` | Thin public host-loader without UI, auth or storage secrets |
| `/widget/cloud-comment-widget-frame.js` | Exact iframe-runtime bundle; not imported into the host-loader |
| `/widget/cloud-comment-widget.css` | Same-origin Shadow DOM stylesheet loaded only by the iframe-runtime |
| `/api/public/sites/{siteId}/widget-frame` | Dynamic no-store frame shell with active-site check and exact per-site `frame-ancestors` CSP |
...
- `data-frame-base-url` — CloudComment-controlled iframe origin; значение или build config обязаны указывать отдельный origin, не совпадающий с origin API; иначе loader завершается fail closed без iframe

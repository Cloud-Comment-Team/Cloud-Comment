...
| Realtime-уведомления | реализовано | `/api/realtime/tickets`, `/api/realtime/ws`, уведомления о новых комментариях |
...
Админка сначала получает одноразовый ticket через защищённый `POST /api/realtime/tickets`, затем открывает `WebSocket /api/realtime/ws?ticket=<one-time-ticket>`. Ticket привязан к текущему пользователю, действует 60 секунд и атомарно погашается при handshake, поэтому долгоживущий bearer token не попадает в URL и proxy logs.
...
События owner-scoped: backend отправляет `comment.created` только активным сессиям владельца из `sites.owner_id`.
